...
def _score_regions_to_genes(
    ...
    # Ensure region names in the search space have the 'chr' prefix
    search_space['Name'] = search_space['Name'].apply(
        lambda x: x if x.startswith('chr') else f'chr{x}'
    )

    # Filter the search space to only include matching regions and genes
...
def calculate_regions_to_genes_relationships(
    ...
    log.info(f"Expression matrix shape: {df_exp_mtx.shape}")
    log.info(f"Accessibility matrix shape: {df_acc_mtx.shape}")
    log.info(f"Search space shape: {search_space.shape}")
    log.info(f"First few rows of search space:\n{search_space.head()}")
    ...
    # log.info(f"Region-to-gene importances: {region_to_gene_importances}")
    log.info(f'Number of genes in region_to_gene_correlation {len(region_to_gene_correlation.keys())}')
    
    log.info(f"Number of genes in region_to_gene_importances: {len(region_to_gene_importances.keys())}")
    if region_to_gene_importances:
        example_gene = next(iter(region_to_gene_importances.keys()))
        log.info(f"Example gene: {example_gene}")
        log.info(f"Regions for example gene: {region_to_gene_importances[example_gene].index}")
