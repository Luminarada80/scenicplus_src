...
def _run_dem_single_region_set(
    ...
    # foreground_regions.df.to_csv(
    #     '/gpfs/Labs/Uzun/SCRIPTS/PROJECTS/2024.GRN_BENCHMARKING.MOELLER/SCENIC_PLUS/input/foreground_regions.csv', 
    #     index=False
    # )
    # background_regions.df.to_csv(
    #     '/gpfs/Labs/Uzun/SCRIPTS/PROJECTS/2024.GRN_BENCHMARKING.MOELLER/SCENIC_PLUS/input/background_regions.csv', 
    #     index=False
    # )
    ...
    dem_result = DEM(
        ...
        motif_similarity_fdr = motif_similarity_fdr,
        orthologous_identity_threshold = orthologous_identity_threshold)
    # print(f"Foreground sets: {foreground_region_sets}")
    # print(f"Background sets: {background_region_sets}")
    # print(f"DEM DB: {dem_db_fname}")
    # print(f"Motif annotations: {path_to_motif_annotations}")
    import pandas as pd
...
def run_motif_enrichment_dem(
    ...
    print(f"Number of DEM results: {len(dem_results)}")        
    for i, dem_result in enumerate(dem_results):
        if len(dem_result.motif_enrichment) > 0:
            log.info(f"Motif enrichment size: {len(dem_result.motif_enrichment)}")
            if i == 0:
                selected_mode = "w"
            else:
                selected_mode = "a"
