...
def _read_motif_hits(h5: h5py.Group) -> Dict[str, Dict[str, List[str]]]:
    ...
    return motif_hits
#@track
def _signatures_to_iter(paths_to_motif_enrichment_results: List[str]):
    """Helper function to iterate over motif enrichment results."""
    for f in paths_to_motif_enrichment_results:
        print(f"Processing file: {f}")  # Debugging
        if f.endswith(".hdf5"):
            try:
                with h5py.File(f, "r") as h5:
                    for name in h5:
                        print(f"Processing group: {name}")  # Debugging
                        if "motif_enrichment" not in h5[name]:
                            continue
                        motif_enrichment = pd.read_hdf(f, key=f"{name}/motif_enrichment")
                        motif_hits = _read_motif_hits(h5[name])
                        yield motif_enrichment, motif_hits["region_set"]
            except OSError as e:
                print(f"Error opening file {f}: {e}")  # Debugging
                raise
#@track
def _get_cistromes(
...
def _cistromes_to_adata(cistromes: List[Cistrome]) -> anndata.AnnData:
    ...
    Parameters
    ----------
    cistromes : List[Cistrome]
        A list of cistrome objects.

    Returns
    -------
    anndata.AnnData
        An AnnData object containing cistrome information.
    """
    # Extract transcription factor names
    tf_names = [cistrome.tf_name for cistrome in cistromes]

    # Join motifs into a string for compatibility with h5ad format
    motifs = [",".join(cistrome.motifs) for cistrome in cistromes]

    # Union of target regions across all cistromes
    union_target_regions = list(
        set().union(*[cistrome.target_regions for cistrome in cistromes])
    )

    # Initialize a hit matrix for target regions
    cistrome_hit_mtx = np.zeros(
        (len(union_target_regions), len(tf_names)), dtype=bool
    )

    # Populate the hit matrix
    for i, cistrome in enumerate(cistromes):
        cistrome_hit_mtx[:, i] = [
            region in cistrome.target_regions for region in union_target_regions
        ]

    # Create an AnnData object with the hit matrix
    cistrome_adata = anndata.AnnData(
        X=sparse.csc_matrix(cistrome_hit_mtx),  # Sparse format for efficiency
        dtype=bool,
        obs=pd.DataFrame(index=list(union_target_regions)),  # Observations: target regions
        var=pd.DataFrame(index=tf_names),  # Variables: transcription factors
    )

    # Add motifs as a variable annotation
